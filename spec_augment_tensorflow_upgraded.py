...
def sparse_warp(spectrogram, time_warping_para=8):
    ...
    warped_image, _ = tfa.image.sparse_image_warp(spectrogram, source_control_point_locations, dest_control_point_locations)
...
def spec_augment(spectrogram):
    ...
    warped_frequency_spectrogram = frequency_masking(warped_spectrogram, v=v)
...
# def visualization_spectrogram(spectrogram, title):
...
#     librosa.display.specshow(librosa.power_to_db(spectrogram[0, :, :, 0], ref=np.max), y_axis='mel', fmax=8000, x_axis='time')
#     plt.title(title)
#     plt.tight_layout()
#     plt.show()


def visualization_tensor_spectrogram(spectrogram, title):
    """visualizing first one result of SpecAugment
